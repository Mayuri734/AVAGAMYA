...
JARGON_DATABASE = {
    # English Jargon
    "Penalties & Fees": ["penalty", "forfeit", "forfeiture", "levy", "late fee", "over-limit", "surcharge"],
    "Exclusion & Liability": ["not liable", "no liability", "unlimited liability", "exclusive remedy", "indemnify", "indemnification", "hold harmless", "disclaimer"],
    "Unilateral Rights": ["sole discretion", "absolute discretion", "at its discretion", "reserve the right", "may refuse", "without notice", "revoke", "nullify"],
    "Legal & Dispute": ["arbitration", "jurisdiction", "governing law", "breach", "default", "binding", "waiver"],
    # Marathi Risk Factors (Translated)
    "Marathi Risk": ["जबाबदार", "दंड", "नुकसानभरपाई", "रद्द", "अस्वीकार", "अटी", "दायित्व", "बंधने", "अधिकार", "क्षेत्राधिकार", "थकीत", "अंतिम"],
    # Hindi Risk Factors (Translated)
    "Hindi Risk": ["उत्तरदायी", "जुर्माना", "हर्जाना", "रद्द", "अस्वीकृत", "शर्तें", "प्रतिबंध", "बकाया", "अनिवार्य"]
}

FORBIDDEN_JARGON_LIST = [term for category in JARGON_DATABASE.values() for term in category]
...
def advanced_clean(text: str) -> str:
    ...
    for line in text.split('\n'):
        ...
        if len(line) > 0 and digit_currency_chars / len(line) > 0.30: continue
        if re.search(r'\b(page|pages?)\s+\d+|\bwww\.|\.com\b|toll\s*free|helpline|contact\s*us', line, re.IGNORECASE): continue
        if line.isupper() and len(line.split()) < 10: continue
        if re.search(r"₹\s*[A-Za-z0-9]+(?:\s*-\s*[A-Za-z0-9]+)+", line) or re.search(r"(?:[_.X]{2,}\s*)+", line): continue
        cleaned_lines.append(line)
        
    return '\n'.join(cleaned_lines)

def semantic_validator(clause: str) -> bool:
    if not clause or not clause.strip(): return False
    try:
        doc = nlp(clause)
        has_verb, has_subject = False, False
        for token in doc:
            if token.pos_ == "VERB": has_verb = True
            if token.dep_ in ("nsubj", "nsubjpass"): has_subject = True
