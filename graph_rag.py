...
def build_knowledge_graph(clauses: List[Dict]) -> nx.Graph:
    ...
    for i in range(len(clauses)):
        for j in range(i + 1, len(clauses)):
            ...
            shared_keywords = {k for k in shared_keywords if k not in ['the bank', 'this agreement', 'the customer', 'your account']}
...
def traverse_graph(clauses: List[Dict], keyword: str) -> str:
    ...
    for start_node in start_nodes:
        ...
        # Get the start node and all its immediate neighbors (1-hop chain reaction)
        subgraph_nodes = [start_node] + list(G.neighbors(start_node))
