...
async def get_raw_clauses(document_hash: str) -> List[Dict]:
    ...
    try:
        if document_hash:
            response = supabase.table("document_cache").select("ai_results").eq("file_hash", document_hash).eq("language", "en").limit(1).execute()
            if not response.data:
                response = supabase.table("document_cache").select("ai_results").eq("file_hash", document_hash).limit(1).execute()
        else:
...
async def search_analyzed_clauses(document_hash: str, query: str) -> str:
    ...
    try:
        ...
        if "penalty" in query_lower or "penalties" in query_lower or "fee" in query_lower or "fees" in query_lower:
            penalties = [c for c in clauses if "fee" in c.get('original_text', '').lower() or "penalty" in c.get('original_text', '').lower()]
            if penalties:
...
async def generate_shrawya_response(query: str, history: List[Dict[str, str]], document_hash: str, language: str = "en") -> str:
...
If the user asks a math/calculation question about loans (e.g. "what is my EMI if I borrow 50000 for 6 months at 7%"), YOU MUST NOT DO THE MATH YOURSELF. 
...
If the user asks about external laws, RBI rules, or banking regulations (e.g. "Is 500 penalty legal under RBI?"), YOU MUST NOT GUESS.
Instead, you MUST reply with EXACTLY this JSON format and nothing else:
{{"action": "SEARCH", "query": "RBI rules on late payment penalty fees 2024"}}

If the user asks about complex chain reactions, triggers, or specific internal document relationships (e.g. "If I miss a payment, what triggers?"), YOU MUST NOT GUESS OR HALLUCINATE.
Instead, you MUST reply with EXACTLY this JSON format and nothing else:
{{"action": "GRAPH_TRAVERSAL", "keyword": "miss a payment"}}

If the user asks about a penalty, fee, or fine, and you explain it, you MUST append this exact tag at the end of your response to offer them a waiver draft: [ACTION: DRAFT_WAIVER_EMAIL]
    ...
    if "SIMULATE" in ans and "{" in ans:
        try:
            ...
            if data.get("action") == "SIMULATE":
                ...
                tool_msg = f"SIMULATOR RESULTS: EMI is {sim_res['monthly_emi']}, Total Interest is {sim_res['total_interest']}, Processing Fee is {sim_res['processing_fee']}, Total GST is {sim_res['total_gst']}, Total Extra Payable is {sim_res['total_extra_payable']}. Now explain this to the user natively in {target_lang}."
                messages.append({"role": "assistant", "content": ans})
                messages.append({"role": "user", "content": tool_msg})
                final_ans = await _call_sarvam(messages)
                if final_ans:
                    # Append Generative UI Flag for the frontend
                    ui_flag = f"\n\n[UI: SIMULATOR | P={data.get('principal', 0)} | R={data.get('roi', 0)} | T={data.get('tenure_months', 0)} | F={data.get('processing_fee_percent', 0)}]"
    ...
    if "SEARCH" in ans and "{" in ans:
        try:
            ...
            if data.get("action") == "SEARCH":
                ...
                tool_msg = f"WEB SEARCH RESULTS for '{search_query}':\n{search_context}\n\nBased on these search results and the document context, answer the user's question natively in {target_lang}."
                messages.append({"role": "assistant", "content": ans})
                messages.append({"role": "user", "content": tool_msg})
                final_ans = await _call_sarvam(messages)
    ...
    if "GRAPH_TRAVERSAL" in ans and "{" in ans:
        try:
            ...
            if data.get("action") == "GRAPH_TRAVERSAL":
                ...
                tool_msg = f"DETERMINISTIC GRAPH RAG RESULTS for '{keyword}':\n{graph_context}\n\nBased on this deterministic graph traversal, explain the chain reaction to the user natively in {target_lang}."
                messages.append({"role": "assistant", "content": ans})
                messages.append({"role": "user", "content": tool_msg})
                final_ans = await _call_sarvam(messages)
